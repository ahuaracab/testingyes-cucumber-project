...
public class DefinicionDePasosParaRegistro {
    ...
    @Given("^que (.*) ingresa a TestingYes$")
    public void ingresa_a_testingyes(String nombreDeUsuario) {
        usuario.isCalled(nombreDeUsuario);
        usuario.ingresaATestingYes();
    }

    @When("^(.*) ingresa a la pagina de log in$")
    public void ingresa_a_log_in(String nombreDeUsuario) {
        usuario.isCalled(nombreDeUsuario);
        usuario.ingresaALogIn();
    }

    @And("^(.*) se registra con los datos obligatorios: nombre: (.*) , apellido: (.*) , pais: (.*) , direccion: (.*) , ciudad: (.*) , provincia: (.*) , codigo postal: (.*) , telefono: (.*) , email: (.*) , contrasena: (.*) y confirmacion de contrasena: (.*)")
    ...
    @Then("^(.*) deberia ver la confirmacion de registro: (.*)$")
